...

{
	...
	db
			...
			function(transaction) {
				try {
					var str='CREATE TABLE links(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, ';
					str=str+'classId INTEGER NOT NULL , ';
					str=str+'clicked INTEGER NOT NULL , ';
					str=str+'dateCreated TEXT NOT NULL , ';
					str=str+'dateModified TEXT NOT NULL, ';
					str=str+'url TEXT NOT NULL, ';
					str=str+'name TEXT NOT NULL);',
...
function createDBLink(classId, url, name) {

	theTime=getTime();
	//alert('createDBLink() called=' + classId+", url="+url+", name="+name);
	today=getToday();
	theDateTime=today+" "+theTime;
	clicked=0;
	var db = initDB();
	db.transaction(function(tx) {
		try {
			tx.executeSql(
					'INSERT into links (classId, url, name, clicked, dateCreated, dateModified) VALUES (?, ?, ?, ?, ?, ?);',
					[ classId, url, name, clicked, theDateTime, theDateTime ]);
		} catch (err) {
			alert(err);
		}
	});
...
function updateDBLink(linkId, url, name) {

	//alert("update link="+linkId);
	theTime=getTime();
	today=getToday();
	theDateTime=today+" "+theTime;
	var db = initDB();
	db.transaction(function(tx) {
		try {
			tx.executeSql(
					'update links set url=?, name=?, dateModified=? where id=?;',
					[ url, name, theDateTime, linkId ]);
		} catch (err) {
			alert(err);
		}
	});
...
function getAllLinksForClass(classId, callback) {
	...
	db
			.transaction(function(tx) {
				try {
					tx
							.executeSql(
									'SELECT * FROM links where classId=? order by dateCreated  ;',
									[ classId ], callback,
									myTransactionErrorCallback);
				} catch (err) {
					alert(err);
...
function getDBLink(id, callback) {
...
//	alert('getDBNote() called=' + id);
	...
	db.transaction(function(tx) {
		try {
			tx.executeSql('SELECT * FROM links where id=?;', [id], callback,
					myTransactionErrorCallback);
		} catch (err) {
			alert(err);
		}
